...
traefik.ensure:%.ensure: | %.start
	...
	@until test -n "$$(curl -sSL --fail "http://$$($(DOCKER) container port "$(TRAEFIK_SERVICE_NAME)" "8080" 2>/dev/null)" 2>/dev/null)"; do \
		if test -z "$$($(DOCKER) container ls --quiet --filter "status=running" --filter "name=^$(TRAEFIK_SERVICE_NAME)$$" 2>/dev/null)"; then \
...
traefik.list:%.list: | %.ensure
	@printf "Open Traefik: %s or %s\n" "http://$(TRAEFIK_DOMAIN)$(if $(filter-out 80,$(TRAEFIK_HTTP_PORT)),:$(TRAEFIK_HTTP_PORT))" "http://$$($(DOCKER) container port "$(*)" "8080")"
.PHONY: traefik.list
